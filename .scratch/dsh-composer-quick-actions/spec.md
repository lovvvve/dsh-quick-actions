...
- 通过公共 Composer 路径执行 Send Action（发送动作）；
- 每个发送动作独立的确认策略；
- 三种全局快捷动作布局；
...
- **首版只有 Send Action（发送动作）一种动作类型**；不提供 Insert Action（插入动作），因此不存在能力自适应投影。
- 只在 Resident Composer 中渲染；恢复到常驻消息编辑器后自然重新出现。
- 动作内容是静态纯文本，不解释变量、模板或脚本。
- 静态文本可以是 `/` 开头的命令；此类动作为 Command Send Action（命令发送动作），确认默认开启但用户可以关闭。
...
- 运行时变量、模板、任意脚本或生成内容；
- 命令面板、纯键盘宏等非按钮调用界面；
- Insert Action（插入动作）及其兼容性抑制投影、能力检测与撤销/选区语义；首版不提供任何只写入草稿而不提交的动作，该能力待公共 `insertText` 可用后另建独立 effort；
- 参与 DSH 原生 `/`、`@` 候选项 pipeline（`inputTriggers`）或复现其候选菜单；
- 把 `inputActions.insertText` 补丁合并进 DSH 官方发布并记录正式首发版本；该工作未来另建独立 effort。

## 3. 领域行为

领域词汇以根目录 [`CONTEXT.md`](../../CONTEXT.md) 为准。以下两种投影状态必须严格区分：
...
首版没有第三种"因 DSH 能力缺失而从布局省略"的状态：所有动作都是发送动作，其执行只依赖当前 DSH 已公开的 `setDraft` 与 `submit`。
...
interface CustomQuickActionValue {
  kind: 'send'
  label: string
  text: string
  icon?: string
...
- `kind` 是动作类型判别式，首版恒为 `'send'`。它**不是配置项**：预置作者不声明它，管理表单不提供动作类型选择器，用户没有任何途径改变它。规范化统一写出 `kind: 'send'`，因此持久化数据始终携带该标签。
- 保留该判别式是为了让未来引入 Insert Action（插入动作）时不必提升 `schemaVersion`，从而**不需要在升级时改写任何既有用户数据**。它只预留标签，不预留未来动作的完整形状；若插入动作最终需要新增字段，届时仍按第 6.3 节走版本化迁移。
...
- 自定义动作 `enabled: true`；
- 预置动作 `hidden: false`；
- 发送动作 `confirm: true`；Command Send Action（命令发送动作）沿用同一默认值，不另设规则，用户可自行关闭。
...
- 静态文本的首个非空白字符为 `/` 时，该动作是 Command Send Action（命令发送动作）：配置有效，`confirm` 与其他发送动作同规则——默认 `true`，用户可关闭。**规范化不得依据文本改写 `confirm`**：默认值只在创建与克隆时初始化，规范化一律透传既有值，否则用户关闭确认后每次规范重写都会被改回，既破坏用户选择也违反第 5.3 节的幂等要求；
- 动作文本不得包含由目标 DSH 公开保留、会在 `setDraft` 中被移除的引用占位符 code point；包含时配置无效，按字段错误处理。
- ID 不得重复；预置标签和自定义标签可以重复；
- Host `Config.presets` 中出现 `kind` 字段且值不是 `'send'` 时（含 `'insert'`），该预置无效，按第 5.1 节使插件配置加载失败；这是作者错误，必须响亮失败而非静默忽略；
- 已存储用户数据中出现非 `'send'` 的 `kind` 时不按无效处理，按第 5.3 节保留为墓碑；这是由更高版本降级回首版时的真实数据，不得删除或改写；
- 规范化后的动作必须显式包含 `kind: 'send'` 与 `confirm`，自定义动作必须显式包含 `enabled`。
...
- 可以在升级时更新标签、图标和文本；
- 不得改变 `kind` 与 `confirm`；二者共同构成不可改变的安全行为签名；
...
1. 复制克隆时刻的标签、文本、图标和确认策略；克隆命令发送动作时同样按原样复制其确认策略，不重新施加默认值；
...
- `kind` 不是 `'send'` 的已存储自定义动作及其顺序引用同样被保留为墓碑：不显示、不计入总量、不可编辑，且规范化不得删除或改写它们。该情形只在从支持更多动作类型的更高版本降级回首版时出现；升级回该版本后必须原样恢复；
...
- 隐藏和停用动作均计入合计；当前目录中未知的预置墓碑，以及 `kind` 非 `'send'` 的动作墓碑，均不计入。
...
若隐藏或停用后没有 Composer 动作，三种布局仍必须显示紧凑管理入口，不得留下不可操作空区域。
...
- 标签、静态文本、可选 emoji、发送确认设置；
- 表单校验、50 项限制、被动超限警告；
- Settings 只读、写入失败、断线和 revision 冲突状态；
- 对 Command Send Action（命令发送动作）的识别标记与集中说明。

文本被识别为命令发送动作时，表单必须即时给出**警示**（不锁定任何控件）：说明该动作会按命令进入官方裁决路径，说明确认面板不会展示 DSH 原生 `/` 候选菜单，并说明关闭确认后该命令将一键提交且没有任何预览。确认开关始终可编辑；表单不得因文本变化自行改写用户已经设定的 `confirm` 值。
...
首版所有动作都是 Send Action（发送动作），执行只依赖目标 DSH 当前已公开的 `InputActions` 成员；不做插入能力检测，也不存在按能力分支的投影。

- 插件在全部路径中只能消费公共 `InputActions` 和其他已记录的公共契约；不得为焦点、选区、状态判断或提交访问 DOM、Lexical、私有 shell、私有事件、私有 keyboard 或合成输入。
- `setDraft` 只能按第 9.4 节用于通过最终重验的未占用草稿发送；不得用它实现选区插入，也不得用它实现任何"只写草稿而不提交"的动作——首版没有这类动作。
- 不得注册、驱动或复现 DSH 原生 `/`、`@` 候选项 pipeline（`inputTriggers`）。Command Send Action（命令发送动作）只是把静态命令文本交给与发送按钮相同的官方路径，由 DSH 自行裁决。
...
Command Send Action（命令发送动作）受完全相同的前置条件约束，不因文本是命令而放宽任何一条。

### 9.3 确认和最终重验

需要确认的发送动作必须打开面板，显示动作标签和完整待发送文本。打开面板不得修改草稿。

Command Send Action（命令发送动作）默认需要确认，用户可关闭；启用确认时，面板必须在展示完整命令文本之外明确告知：该文本将按命令进入 DSH 官方裁决路径，并且此处**不会**出现原生 `/` 输入时的候选菜单，因此用户看到的就是最终提交内容。这是本项决策已知并接受的取舍，实现不得用任何自制候选菜单模拟原生 pipeline。
...
动作、Session、草稿占用或原生 guard 任一条件变化时，不得修改草稿或发送，并提示“状态已变化，请重试”。动作定义在等待期间被改写，使其跨越 Command Send Action（命令发送动作）边界时，同样按定义变化处理并要求重试。

### 9.4 文本装载与官方提交

最终重验通过后，发送调用公共 `inputActions.setDraft(text)` 装载静态文本，随后立即调用公共 `inputActions.submit()`，进入与 Composer 发送按钮相同的官方 queue、命令裁决、乐观提交和失败恢复流程。首版只有这一条装载路径，不做能力分支。

不得直接调用 Session controller、Host Remote 或私有 keyboard 提交路径。`setDraft` 以最终未占用草稿重验为安全前提，只服务于发送动作。

Command Send Action（命令发送动作）走完全相同的两步；命令的识别、裁决与执行全部由 DSH 官方路径负责，插件不解析、不改写、不预判命令语义。

### 9.5 单飞、失败与反馈

- 同一 Session 从首次激活直到官方提交阶段结束，必须保持单飞；确认取消或入队失败后按既定语义结束单飞。除这些已批准边界外，不得把单飞提前释放到某一未经批准的时间点。
- 单飞窗口只能由目标 DSH 已公开的状态判定。公开 Input snapshot 为 `{ draft, imageIds, draftRev, phase, claim?, occurrences, queue }`，实现必须从中确定可归属于本次提交的最迟可靠边界，并用测试固定该判据；不得使用 DOM、Lexical、私有事件或私有状态兜底。
- 该窗口的硬验收标准是**不产生重复发送**（第 13.1 节列为发布阻断项）。官方 sink 采用乐观清空，`setDraft` + `submit()` 后草稿会在一帧内回到空态，因此"草稿已占用"不足以充当互斥；实现必须覆盖快速连续激活的回归测试。
- `setDraft` 或 `submit` 任一调用在装载前后发生同步异常，或状态机未接收提交时，插件必须结束单飞且不得自动重试。若草稿已经包含本次装载文本，必须原样保留并提示“未发送，文本已保留”；若尚未写入，则保持原草稿并显示可重试执行错误。
- 消息已经进入官方提交路径后的失败完全交给 DSH 原生错误反馈和失败草稿恢复；插件不得自动重试、复制草稿或显示重复错误。
- 成功发送后的本地消息回显就是成功反馈，不得产生额外成功 toast；命令发送后的裁决结果同样由 DSH 原生反馈呈现。
...
- Peer range 覆盖公开 `setDraft` 与 `submit` 的受支持 DSH；首版不依赖 `insertText`，安装与运行都不检测该能力。
- README 必须说明首版只提供发送动作：静态文本经公共 `setDraft` + `submit` 走官方提交路径；不提供插入动作，该能力待公共 `insertText` 可用后另建 effort。
- README 必须单列 Command Send Action（命令发送动作）：`/` 开头文本合法，确认默认开启且可关闭；启用确认时面板展示最终提交内容，但**不会**出现 DSH 原生 `/` 候选菜单；关闭确认后命令一键提交、无预览。命令一律由官方路径裁决。
...
- 公共附件字段下的占用草稿、最终重验、确认取消、动作删除、会话切换和按会话单飞；
- 模型运行期间官方 queue、装载后失败草稿保留及官方失败不重复报错；
- 规范化统一写出 `kind: 'send'`；Host `Config.presets` 中的非 `'send'` `kind` 使配置加载失败；
- 已存储的 `kind: 'insert'` 动作被保留为墓碑：不显示、不计数、不被规范化删除或改写，且在恢复到支持该类型的版本后原样可用（降级往返无损）；
- Command Send Action（命令发送动作）判定：`/` 开头、前置空白后为 `/`、非 `/` 开头；
- **规范化不改写 `confirm`**：用户把命令动作的确认关闭后，重复规范化、重启与预置升级都不得把它改回 `true`；预置声明 `confirm: false` 的命令动作按声明值保留；
- 命令发送动作确认开启时面板包含无候选菜单说明；确认关闭时一键提交且不弹面板；两种情况的执行路径与普通发送动作完全一致；
...
首版只有一个 GUI 通道：使用现有 `http://127.0.0.1:3080` 官方 DSH GUI，验证三种布局、紧凑管理入口、动作管理与持久化、公共 `setDraft` + `submit` 发送路径、命令发送动作在确认开启与关闭两种设置下的提交，以及写入后失败恢复。

因为首版不依赖 `insertText`，**不再需要隔离源码检出、核心补丁应用或第二个受管 GUI 服务器**；该完整能力通道随插入动作一并推迟到未来 effort。

不得修改已安装 `node_modules`。首版自动化使用最终候选环境的当前 Node 版本和 Playwright Chromium；人工检查覆盖桌面、约 768px 和约 360px。首版不宣称覆盖其他浏览器引擎或操作系统。
...
本节由 [HITL 票据 21](./issues/21-run-final-human-acceptance.md) 承担；票据 18 只负责到自动化证据完整为止。没有完整证据和用户最终明确的“生产验收通过”，不得完成 Wayfinder 地图目标。
...
| 集成与发布验证（自动化，AFK） | [18](./issues/18-run-integration-and-release-verification.md) |
| 最终人工验收（HITL，地图收尾） | [21](./issues/21-run-final-human-acceptance.md) |
| 核心 `insertText` 能力基线 | [10](./issues/10-publish-dsh-composer-insert-text-api.md)（已解决；随插入动作一并移出首版范围，保留为未来 effort 资产） |
| 正式上游合并 | [19](./issues/19-upstream-insert-text-and-record-release.md)（已关闭为首版范围外） |
| 首版范围收缩与斜杠命令处置 | 第 16 节 |
...
5. **Submit 接收凭据**：新增最小公共提交凭据，支撑既定单飞窗口，不得用私有状态兜底。**（已由第 16.4 节取代：首版不新增核心接口，改为基于既有公开状态实现。）**
...
- 票据 15：单飞窗口的公开状态判据与测试（凭据方案已由第 16.4 节取代）；Resident Composer 公开判定方案及其 GUI 验证（通道数量见第 16 节，已收敛为单通道）。
...
## 16. 首版范围收缩：移除插入动作与斜杠命令处置

目标 DSH 至今没有公开 `insertText`。用户据此做出以下首版决定。**本节优先于正文其余部分以及第 15 节中与之冲突的表述。**

### 16.1 决定

1. **首版移除 Insert Action（插入动作）**，只交付 Send Action（发送动作）。随之取消：插入能力检测与按能力分支的投影、Compatibility-Suppressed Insert Action（兼容性抑制插入动作）这一概念、发送的双装载路径，以及第二个"完整能力" GUI 验证通道。
2. **保留 `kind` 判别式，首版恒为 `'send'`**。它不是配置项：预置作者不声明、表单不提供选择器、用户无法改变，规范化统一写出该标签。这样未来引入插入动作时既不必提升 `schemaVersion`，也**不必在升级时改写任何既有用户数据**——按第 10 节，规范重写是受 revision fence 保护的写操作，冲突时需要刷新权威快照甚至请用户重做，保留标签即可完全避免这次写入。代价只是一个恒定字段。

   非 `'send'` 值分两种处置：出现在 Host `Config.presets` 中是作者错误，按第 5.1 节使配置加载失败；出现在已存储用户数据中则是更高版本降级回首版的真实数据，按第 5.3 节保留为墓碑（不显示、不计数、不改写），使降级往返无损。

   该保留不预留未来动作的完整形状；若插入动作最终需要新增字段，届时仍按第 6.3 节走版本化迁移。
3. **斜杠命令改为允许，确认默认开启但可关闭**。首个非空白字符为 `/` 的静态文本不再是无效配置，而是 Command Send Action（命令发送动作）。其 `confirm` 沿用发送动作的通用默认值 `true`，用户可以关闭；**规范化不得依据文本改写该值**，默认只在创建与克隆时初始化，否则会毁掉用户选择并破坏规范化幂等。管理表单在文本成为命令时给出警示但不锁定任何控件；确认面板在启用时展示完整命令文本并说明不会出现 DSH 原生候选菜单。

### 16.2 已知并接受的取舍

决定 3 取代了[票据 06](./issues/06-decide-runtime-and-failure-semantics.md)的原结论——"发送文本首个非空白字符为 `/` 则配置无效；命令文本只能通过插入动作让用户检查后手动提交"。该结论把插入动作当作命令的逃生通道，而决定 1 移除了这条通道。

用户已知悉并接受以下代价：命令发送动作经 `setDraft` + `submit` 进入官方命令裁决，但用户在插件确认面板中看到的是纯静态文本，看不到原生 `/` 输入时的候选菜单，因此裁决结果可能与用户在原生输入框逐字输入同一命令时的预期不同。缓解手段是确认默认开启、表单在文本成为命令时给出警示、以及确认面板中的明确说明。用户可以关闭确认，此时命令将一键提交且没有预览——这是用户对自己预先配置的静态命令做出的显式选择，属于已接受范围。不得自制候选菜单、不得解析或改写命令语义、不得注册或驱动 `inputTriggers` pipeline（研究依据见 [`research/composer-extension-seams.md`](./research/composer-extension-seams.md) 第 4.4 节：只有 input pipeline 持有可用的新鲜选区/span）。

### 16.3 票据映射

| 变更 | 票据 |
|---|---|
| 模型恒写 `kind: 'send'`、非 `'send'` 存储数据的墓碑保留、去兼容性抑制投影、命令发送动作判定、规范化对 `confirm` 透传 | 12 |
| Host Config 中非 `'send'` `kind` 使配置加载失败、命令预置的规范化 | 13 |
| 去能力检测与插入执行、单一装载路径、`Blocked by` 去 10 | 15 |
| 管理表单去动作类型、命令动作警示与集中说明 | 16 |
| README 去能力矩阵、新增命令动作说明 | 17 |
| 去完整能力通道、去插入验收步骤、新增命令动作验收、`Blocked by` 去 10 | 18 |
| 不受影响 | 11、14、19、20 |

票据 06、08、09 已 resolved，**不重开**；其中与本节冲突的插入动作与斜杠命令结论以本节为准，各票据在 `## Comments` 下记录取代关系。
