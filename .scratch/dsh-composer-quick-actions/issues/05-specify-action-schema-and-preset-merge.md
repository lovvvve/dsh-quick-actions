...
内置与 Host 配置组成的预置目录本身最多包含 50 个动作；超过时配置加载失败。正常状态下，预置与自定义动作合计最多 50 个，隐藏和停用的动作仍计入总数，新增或克隆不得使合计主动超限。若软件包升级新增预置而使已有状态被动超过 50，不删除、不隐藏也不拒绝加载任何已有动作；管理界面显示超限警告并禁止新增或克隆，直到删除动作或后续预置移除使合计恢复到 50 以内。该发布边界由[定义验证与发布验收标准](./08-define-verification-and-release-acceptance.md)确认。
...
- 「插入/发送判别联合」与「`kind` 属于不可改变的安全行为签名」：**仍然有效**。首版保留 `kind` 判别式，只是把它固定为 `'send'` 且不开放为配置项（规范化统一写出该标签），以便未来加入插入动作时无需提升 `schemaVersion`、无需改写既有用户数据。
- 行为签名新增一条：不得通过改写文本让动作跨越 Command Send Action（命令发送动作）边界，需要改变时必须换新的 Preset Action ID。
- 稳定双来源身份、不可变预置与用户增量、统一引用顺序、确定性规范化与版本化存储：**仍然有效**。
