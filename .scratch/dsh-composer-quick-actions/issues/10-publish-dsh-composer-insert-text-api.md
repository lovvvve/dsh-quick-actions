...
本票据保持 resolved，补丁与验证记录全部保留，但**已移出首版关键路径**：首版不提供插入动作，因此不应用此补丁、不创建隔离源码检出、不启动第二个 GUI 服务器。票据 15、18 的 `Blocked by` 已相应移除 `10`。补丁作为未来插入动作 effort 的资产继续有效，仍不得冒称为正式 DSH 发布版本。
