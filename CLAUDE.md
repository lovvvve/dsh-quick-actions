...
- **包已发布，市场安装通道已打通**：`dsh-quick-actions` 的 `dist-tags.latest` 是 `0.1.0`（2026-09-16T08:49:13Z 发布；此前有 `0.1.0-rc.3`、`0.1.0-rc.4`）。早先 `0.1.0-rc.1` 发出后被撤回，该「名字 + 版本」组合永久作废并触发过一段撤回冷却，冷却已于 2026-09-11 过去；再发一版只需 `pnpm --filter dsh-quick-actions publish`（需 OTP）。**不要再撤回任何东西**，否则冷却窗口从最后一次撤回重新计时。包始终不设 `publishConfig`（无 scope 的包默认就是 public）。**发布后别急着判定失败**：packument 落库实测滞后 30 秒到 2.5 分钟，`npm view` 在滞后期误报 404；对撤回过的包名，首次重发还会报 `[E409] Failed to save packument` 却其实已落库——一律先查 `curl -s https://registry.npmjs.org/dsh-quick-actions` 的 `versions`，不要盲目重发。另注意 **pnpm 的 `minimumReleaseAge`（默认 24 小时）在存在更旧合格版本时会静默退回旧版**而不是报错，所以新版发布后的头一天，`dsh plugin add` 装到的仍是上一版，这是正常的、会自动收敛。**发布 manifest 必须带 `repository`（含子包 `directory`）**：策展仓库 `awesome-dsh-plugin` 的 npm 映射不接受手写，由 `probe-npm.mjs` 拿 `dist-tags.latest` 那份 manifest 的 `repository` 反向核对仓库归属，缺了就判「没发 npm」、市场退回**永远装不成**的 GitHub 源（票据 30）；`tests/release/packaging.spec.ts` 的 `release identity` 已把它钉成打包契约，别当成可有可无的元数据删掉。
