...
- `issues/NN-*.md`：开工前把 `Status:` 设为 `claimed`，完成时追加 `## Answer` 并设 `resolved`，再回填地图。frontier = 开放、未阻塞、未认领中编号最小者。**01 至 26、28 至 30 全部 resolved**，地图目标已达成；当前 frontier 是[票据 27 发布到 npm 并在插件市场上架](.scratch/dsh-composer-quick-actions/issues/27-publish-to-npm-and-list-in-market.md)（已认领；npm 撤回冷却早已过去，rc.4 在架、市场安装已由票据 30 验通，余下 `0.1.0` 正式版与上架条目维护）。票据 26 把管理面板改经 `createPortal` 挂到 `document.body`；**只做了管理面板**，两个锚定 popover（`ActionPanel` / `ConfirmPanel`）仍在 dock 子树内、带着同一个层叠根因，只是尚无用户可见症状——要动它们须先解决锚定定位改用视口坐标的问题，且应另开票据。
...
- **首版不新增任何 DSH 核心接口**。`insertText` 和 submit 凭据都不做。单飞窗口只能用公开 Input snapshot（`draft/attachmentIds/draftRev/phase/claim?/occurrences/queue`——DSH 0.1.5-rc.1 把 `imageIds` 改名为 `attachmentIds`，见 spec 第 21 节与[票据 29](.scratch/dsh-composer-quick-actions/issues/29-follow-dsh-input-contract-change.md)）判定，硬标准是不产生重复发送——注意官方 sink 乐观清空，`submit()` 后草稿一帧内就空了，"草稿已占用"不是互斥锁。
...
- **包已发布，市场安装通道已打通**：`dsh-quick-actions` 的 `dist-tags.latest` 是 `0.1.0-rc.4`（2026-09-13T04:29:19Z 发布）。早先 `0.1.0-rc.1` 发出后被撤回，该「名字 + 版本」组合永久作废并触发过一段撤回冷却，冷却已于 2026-09-11 过去；再发一版只需 `pnpm --filter dsh-quick-actions publish`（需 OTP）。**不要再撤回任何东西**，否则冷却窗口从最后一次撤回重新计时。包始终不设 `publishConfig`（无 scope 的包默认就是 public）。**发布 manifest 必须带 `repository`（含子包 `directory`）**：策展仓库 `awesome-dsh-plugin` 的 npm 映射不接受手写，由 `probe-npm.mjs` 拿 `dist-tags.latest` 那份 manifest 的 `repository` 反向核对仓库归属，缺了就判「没发 npm」、市场退回**永远装不成**的 GitHub 源（票据 30）；`tests/release/packaging.spec.ts` 的 `release identity` 已把它钉成打包契约，别当成可有可无的元数据删掉。
