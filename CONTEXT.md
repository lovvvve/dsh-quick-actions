...
当前 DSH 已具备所需执行能力，但动作因当前草稿、消息编辑器状态或其他临时 guard 不能执行；它仍显示在消息编辑器中并说明原因。
_避免使用_：隐藏动作、已停用动作、Compatibility-Suppressed Insert Action（兼容性抑制插入动作）

**Compatibility-Suppressed Insert Action（兼容性抑制插入动作）**：
因当前 DSH 不具备受支持的纯文本插入能力而不进入常驻消息编辑器布局的 Insert Action（插入动作）；它仍可在管理界面配置、计入动作总量上限，并在该能力可用后自动恢复。
_避免使用_：Hidden Quick Action（隐藏快捷动作）、Unavailable Quick Action（暂不可用快捷动作）、禁用插入动作
